...
export const zodEnv = z
  .object({
    ...
    // Locale
    NEXT_PUBLIC_LOCALE: z.enum(['en']).optional(),
    ...
    FIL_DC_PRICE: z.string().transform(val => parseFloat(val)).refine(val => !isNaN(val), {
      message: 'FIL_DC_PRICE must be a decimal number',
    }),
  })
  .refine((data) => {
